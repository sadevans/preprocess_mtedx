...
class VideoProcess:
    ...
    def __call__(self, video, landmarks):
        ...
        if preprocessed_landmarks is None:
            return 
        elif not preprocessed_landmarks:
            return
    ...
    def crop_patch(self, video, landmarks):
        ...
        for frame_idx, frame in enumerate(video):
            ...
            # print(len(smoothed_landmarks))
            transformed_frame, transformed_landmarks = self.affine_transform(
                frame, landmarks[frame_idx], self.reference, grayscale=self.convert_gray
            )
    ...
    def interpolate_landmarks(self, landmarks):
        ...
        if len(valid_frames_idx) != len(landmarks):
            landmarks = None
        else:
