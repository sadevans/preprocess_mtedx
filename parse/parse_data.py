...
def get_vtt(root_dir, dataset, src_lang):
    ...
    for sub in SUBSETS:
        vtt_path = os.path.join(root_dir, dataset, src_lang+"-"+src_lang, "data", sub, "vtt")
        wav_path = os.path.join(root_dir, dataset, src_lang+"-"+src_lang, "data", sub, "wav")
        yaml_sub_path = os.path.join(root_dir, dataset, src_lang+"-"+src_lang, "data", sub, "txt", sub + '.yaml')
        segm_sub_path = os.path.join(root_dir, dataset, src_lang+"-"+src_lang, "data", sub, "txt", "segments")

        if not path.exists(yaml_sub_path): sub_yaml = open(yaml_sub_path, "w")
        else: sub_yaml = open(yaml_sub_path, "a")

        if not path.exists(segm_sub_path): sub_segm = open(segm_sub_path, "w")
        else: sub_segm = open(segm_sub_path, "a")
        ...
        for file in files:
            ...
            if not path.exists(os.path.join(vtt_path, name + '.vtt')):
                ...
                try:
                    ...
                    continue
                with open(os.path.join(vtt_path, name + '.vtt'), "w") as f:
                    f.write(vtt)
                if flag_open:
                    for sentence in transcript.get_sentences():
                        sent_file.write("{}\n".format(f"{sentence.text}"))
                        data = {'duration': sentence.end - sentence.start, 'offset': sentence.start, 'speaker_id': name, 'wav': name+'.flac'}
                        sub_segm.write("{}\n".format(f"{sentence.text}"))
                        yaml.dump([data], sub_yaml, default_flow_style=None)
                    flag_open = False
                else:
                    sent_file = open(sentences_file_path, "a")
                    for sentence in transcript.get_sentences():
                        sent_file.write("{}\n".format(f"{sentence.text}"))
                        data = {'duration': sentence.end - sentence.start, 'offset': sentence.start, 'speaker_id': name, 'wav': name+'.flac'}
                        sub_segm.write("{}\n".format(f"{sentence.text}"))
...
if __name__ == "__main__":
    ...
    #     video_urls = get_channel_videos(src_lang=args.src_lang, count_vids=args.count,channel_name=channel_name)
    # if args.query: 
    #     query = args.query
    #     video_urls = search_videos(src_lang=args.src_lang, count_vids=args.count, query=query)
    ...
    # for sub in SUBSETS:
    #     os.makedirs(os.path.join(args.root_dir, args.dataset, args.src_lang+"-"+args.src_lang, "data", sub, "txt"), exist_ok=True)
    #     os.makedirs(os.path.join(args.root_dir, args.dataset, args.src_lang+"-"+args.src_lang, "data", sub, "vtt"), exist_ok=True)
    #     os.makedirs(os.path.join(args.root_dir, args.dataset, args.src_lang+"-"+args.src_lang, "data", sub, "video"), exist_ok=True)
    #     os.makedirs(os.path.join(args.root_dir, args.dataset, args.src_lang+"-"+args.src_lang, "data", sub, "wav"), exist_ok=True)
    #     sub_file_path = os.path.join(args.root_dir, args.dataset, args.src_lang+"-"+args.src_lang, "data", sub, "txt", "videos")
    #     sub_file = open(sub_file_path, "w")
    #     if sub == "train":
    #         for url in train_urls:
    #             sub_file.write("{}\n".format(f"{url}"))
    ...
    # transcriber = aai.Transcriber(config=config)
    os.makedirs(os.path.join(args.root_dir, args.dataset, args.src_lang+"-"+args.src_lang, "doc"), exist_ok=True)
    # sentences_file_path = os.path.join(args.root_dir, args.dataset, args.src_lang+"-"+args.src_lang, "doc", f"input.{args.src_lang}")
    # sent_file = open(sentences_file_path, "w")
    # flag_open = True
#     for sentence in sentences:
#   print(sentence.text)
    # download_audio_video(args)
    get_vtt(args)
    # for sub in SUBSETS:
    #     sub_file_path = os.path.join(args.root_dir, args.dataset, args.src_lang+"-"+args.src_lang, "data", sub, "txt", "videos")
    #     sub_file = open(sub_file_path, "r").readlines()
    #     # print(sub_file)
    #     download_path = os.path.join(args.root_dir, args.dataset, args.src_lang+"-"+args.src_lang, "data", sub, "video")
    #     vtt_path = os.path.join(args.root_dir, args.dataset, args.src_lang+"-"+args.src_lang, "data", sub, "vtt")
    ...
    #             download = download_video_from_youtube(download_path, video_id)
                # if download:
                #     transcript = transcriber.transcribe(os.path.join(download_path, video_id + '.mp4'))
                #     vtt = transcript.export_subtitles_vtt()
                #     with open(os.path.join(vtt_path, video_id + '.vtt'), "w") as f:
                #         f.write(vtt)
                #     if flag_open:
                #         for sentence in transcript.get_sentences():
                #             sent_file.write("{}\n".format(f"{sentence.text}"))
...
# python test1.py --root-dir "/media/sadevans/T7 Shield/PERSONAL/Diplom/datasets" --dataset "Vmeste" --channel "@vmeste.project" --src-lang "ru"