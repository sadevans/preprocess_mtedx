...
def preprocess(args, subset, lang):
    ...
    another_label_filename = os.path.join(
        args.root_dir,
        "labels",
        f"{args.dataset}_{args.subset}_transcript_lengths_seg{args.seg_duration}s_{args.units}units_online.csv"
        if args.groups <= 1
        else f"{args.dataset}_{args.subset}_transcript_lengths_seg{args.seg_duration}s_{args.units}units_online.{args.groups}.{args.job_index}.csv",
    )
